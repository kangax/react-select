   ...
   */
	...
	getStateFromValue: function getStateFromValue(value, options, placeholder) {
		...
		return {
			...
			placeholder:
				!this.props.multi && values.length ? values[0][this.props.labelKey] : placeholder,
	...
	initValuesArray: function initValuesArray(values, options) {
		...
		if (!Array.isArray(values)) {
			if (typeof values === 'string') {
				values =
					values === '' ? [] : this.props.multi ? values.split(this.props.delimiter) : [values];
			} else {
			...
			if (typeof val === 'string' || typeof val === 'number') {
				...
				for (var key in options) {
					if (
						...
						options[key] &&
						(options[key][_this5.props.valueKey] === val ||
							(typeof options[key][_this5.props.valueKey] === 'number' &&
								options[key][_this5.props.valueKey].toString() === val))
					) {
						return options[key];
					}
				}
				return (
					(_ref = {}),
					_defineProperty(_ref, _this5.props.valueKey, val),
					_defineProperty(_ref, _this5.props.labelKey, val),
					_ref
				);
			} else {
				return val;
	...
	buildMenu: function buildMenu() {
		...
		// Force re-filtering in case options changed
	...
	render: function render() {
		...
		// clear "x" button
		var clear =
			this.props.clearable && this.state.value && !this.props.disabled && !this.isLoading()
				? React.createElement(
						'span',
						{
		...
		if (!this.props.disabled) {
			if (this.props.searchable) {
				input = React.createElement(
					Input,
					_extends(
						{ value: this.state.inputValue, onChange: this.handleInputChange, minWidth: '5' },
						inputProps
					)
				);
			} else {
